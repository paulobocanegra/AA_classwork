...
def harrison_ford
  ...
  # ord in castings connected to leading status, 1 is leading
  # join movies w/ castings & then w/ actors to get names


  Movie.joins(:actors).where( actors: {name: "Harrison Ford"}).where("ord > 1").select(:id, :title)
 
  #RAW SQL
  # SELECT "movies".* FROM "movies" INNER JOIN "castings" ON "castings"."movie_id" = "movies"."id" INNER JOIN "actors" ON "actors"."id" = "castings"."actor_id"
...
def biggest_cast
  ...
  # SELECT  "movies"."id", "movies"."title" FROM "movies" INNER JOIN "castings" ON "castings"."movie_id" = "movies"."id" GROUP BY "movies"."id" ORDER BY COUNT(actor_id) DESC LIMIT $1  [["LIMIT", 3]]
...
def directed_by_one_of(them)  #
  ...
  # ....IN <SUBQUERY THAT GETS LIST OF FILMS DIRECTED BY THOSE GUYS>

  dirs = Movie.director_id.includes()
  them.each do |dir| 
    Movie.select(:id, :title).where(director: them)
